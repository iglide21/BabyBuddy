...
import { Plus, Baby, Moon, Milk, Calendar, Settings, History, Edit, Bell } from "lucide-react"
import { LogFeedingModal } from "./components/log-feeding-modal"
import { LogSleepModal } from "./components/log-sleep-modal"
import { HistoryView } from "./components/history-view"
import { SettingsView } from "./components/settings-view"
import { LogDiaperModal } from "./components/log-diaper-modal"
import { EditFeedingModal } from "./components/edit-feeding-modal"
import { EditSleepModal } from "./components/edit-sleep-modal"
import { EditDiaperModal } from "./components/edit-diaper-modal"
...
export default function BabyBuddyApp() {
  const [currentView, setCurrentView] = useState<"dashboard" | "history" | "settings" | "notifications">("dashboard")
  ...
  const editFeedingLog = (log: FeedingLog) => {
    setFeedingLogs((prev) => prev.map((item) => (item.id === log.id ? log : item)))
  }

  const editSleepLog = (log: SleepLog) => {
    setSleepLogs((prev) => prev.map((item) => (item.id === log.id ? log : item)))
  }

  const editDiaperLog = (log: DiaperLog) => {
    setDiaperLogs((prev) => prev.map((item) => (item.id === log.id ? log : item)))
  }
  ...
  const todayFeedings = feedingLogs.filter((log) => log.timestamp >= todayStart && log.timestamp < todayEnd)

  const todaySleep = sleepLogs.filter((log) => log.startTime >= todayStart && log.startTime < todayEnd)

  const todayDiapers = diaperLogs.filter((log) => log.timestamp >= todayStart && log.timestamp < todayEnd)
  ...
  const totalSleepHours = Math.round((totalSleepToday / (1000 * 60 * 60)) * 10) / 10
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-blue-50 to-yellow-50">
        ...
        <div className="grid grid-cols-3 gap-3">
          <Card className="bg-orange-50 border-orange-200">
            <CardContent className="p-4 text-center">
              <div className="flex items-center justify-center gap-2 mb-2">
                ...
                <span className="text-sm font-medium text-orange-800">Feedings Today</span>
              </div>
              <div className="text-2xl font-bold text-orange-700">{totalFeedingsToday}</div>
              {lastFeeding && (
                <div className="text-xs text-orange-600 mt-1">Last: {getTimeSince(lastFeeding.timestamp)}</div>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="bg-blue-50 border-blue-200">
            <CardContent className="p-4 text-center">
              <div className="flex items-center justify-center gap-2 mb-2">
                ...
                <span className="text-sm font-medium text-blue-800">Sleep Today</span>
              </div>
              <div className="text-2xl font-bold text-blue-700">{totalSleepHours}h</div>
              {lastSleep && (
                <div className="text-xs text-blue-600 mt-1">
                  {lastSleep.endTime ? `Last: ${getTimeSince(lastSleep.endTime)}` : "Sleeping now 😴"}
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="bg-green-50 border-green-200">
            <CardContent className="p-3 text-center">
              ...
              <div className="text-xl font-bold text-green-700">{totalDiapersToday}</div>
              {lastDiaper && (
                <div className="text-xs text-green-600 mt-1">Last: {getTimeSince(lastDiaper.timestamp)}</div>
              )}
            </CardContent>
          </Card>
        </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <div className="font-medium text-blue-800">Sleep in progress</div>
                    <div className="text-sm text-blue-600">Started at {formatTime(activeSleep.startTime)}</div>
                  </div>
                </div>
                <Button
                  onClick={() => {
          ...
          <CardContent className="space-y-3">
            {todayActivities.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                ...
                <p className="text-xs mt-1">Tap the buttons above to get started! 🍼</p>
              </div>
            ) : (
              <>
                      ...
                      <>
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            {!activity.endTime && <Badge className="bg-blue-100 text-blue-700">Active</Badge>}
                          </div>
                          <div className="text-sm text-gray-600">
          ...
          <Card className="bg-gradient-to-r from-pink-50 to-purple-50 border-pink-200">
            <CardContent className="p-4 text-center">
              <p className="text-sm text-gray-700">
                {totalFeedingsToday >= 6 && totalSleepHours >= 10 && totalDiapersToday >= 6
      ...
      {/* Add Modals */}
      <LogFeedingModal open={showFeedingModal} onClose={() => setShowFeedingModal(false)} onSave={addFeedingLog} />
      ...
      <LogDiaperModal open={showDiaperModal} onClose={() => setShowDiaperModal(false)} onSave={addDiaperLog} />
