...
import { ArrowLeft, Bell, Plus, Trash2, Clock, Milk, Moon, AlertCircle } from "lucide-react"
...
export function NotificationsView({ onBack }: NotificationsViewProps) {
  ...
  const getNextTrigger = (reminder: Reminder) => {
    ...
    } else if (reminder.type === "interval" && reminder.intervalHours && reminder.lastTriggered) {
      return new Date(reminder.lastTriggered.getTime() + reminder.intervalHours * 60 * 60 * 1000)
    }
    return null
  }
      ...
      <div className="max-w-md mx-auto px-4 py-6 space-y-6">
          ...
          <CardContent className="space-y-3">
            {reminders.filter((r) => r.enabled).length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                ...
                <p className="text-xs mt-1">Add some reminders to stay on track! 🔔</p>
              </div>
            ) : (
              reminders
                .filter((r) => r.enabled)
                .map((reminder) => {
                  ...
                  return (
                    <div key={reminder.id} className="flex items-center gap-3 p-3 rounded-lg bg-gray-50">
                      <div
                        className={`w-10 h-10 rounded-full flex items-center justify-center ${getActivityColor(reminder.activity)}`}
                      >
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          <Badge variant="secondary" className="text-xs">
                            {reminder.type === "scheduled" ? "Scheduled" : "Interval"}
                          </Badge>
                        </div>
                        <div className="text-sm text-gray-600">
                          {reminder.type === "scheduled" && reminder.time && <>Daily at {reminder.time}</>}
                          {reminder.type === "interval" && reminder.intervalHours && (
                            <>Every {reminder.intervalHours} hours</>
                          )}
                          {nextTrigger && (
                            <span className="ml-2 text-purple-600">• {formatTimeUntil(nextTrigger)}</span>
                          )}
                        </div>
                      </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              {reminders
                .filter((r) => !r.enabled)
                .map((reminder) => (
                  <div key={reminder.id} className="flex items-center gap-3 p-3 rounded-lg bg-gray-50 opacity-60">
                    <div
                      className={`w-10 h-10 rounded-full flex items-center justify-center ${getActivityColor(reminder.activity)}`}
                    >
                      {getActivityIcon(reminder.activity)}
        ...
        <Card className="bg-gradient-to-r from-purple-50 to-blue-50 border-purple-200">
          <CardContent className="p-4">
            <h3 className="font-medium text-gray-800 mb-2">🔔 About Notifications</h3>
            <ul className="text-sm text-gray-600 space-y-1">
              <li>
                • <strong>Scheduled:</strong> Reminders at specific times daily
              </li>
              <li>
                • <strong>Interval:</strong> Reminders based on time since last activity
              </li>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value: any) => setNewReminder((prev) => ({ ...prev, activity: value }))}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            {newReminder.type === "interval" && (
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setNewReminder((prev) => ({ ...prev, intervalHours: Number.parseInt(e.target.value) }))
                  }
                />
              </div>
            )}
            ...
            <div className="flex gap-3 pt-4">
              ...
              <Button onClick={addReminder} className="flex-1 bg-purple-500 hover:bg-purple-600 text-white">
                Add Reminder
              </Button>
            </div>
          </div>
