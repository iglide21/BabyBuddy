...
export function AIChat({ feedingLogs, sleepLogs, diaperLogs, babyName, birthDate }: AIChatProps) {
  ...
  const generateAIResponse = (userMessage: string): string => {
    ...
    const recentFeedings = feedingLogs.filter((log) => log.timestamp >= weekAgo)
...
**Sleep Patterns:**
• Average sleep duration: ${Math.round(avgSleepDuration * 10) / 10} hours per session
...
${babyName}'s current pattern looks ${avgFeedingsPerDay >= 6 ? "great! This is within normal range." : "like it might need attention. Consider consulting your pediatrician if you're concerned."}`
      }

      if (message.includes("amount") || message.includes("how much")) {
        const bottleFeedings = recentFeedings.filter((f) => f.type === "bottle" && f.amount)
        const avgAmount =
          bottleFeedings.length > 0
            ? bottleFeedings.reduce((sum, f) => sum + (f.amount || 0), 0) / bottleFeedings.length
            : 0

        return `🍼 ${babyName}'s average bottle feeding: ${Math.round(avgAmount * 10) / 10}oz
...
What specific feeding question can I help with?`
    ...
    if (message.includes("sleep") || message.includes("nap") || message.includes("bedtime")) {
      if (message.includes("how long") || message.includes("duration")) {
        return `😴 ${babyName}'s average sleep session: ${Math.round(avgSleepDuration * 10) / 10} hours
...
${avgSleepDuration >= 2 ? "That's a good duration!" : "Short naps are normal for young babies."} 
...
What specific sleep challenge can I help with?`
    ...
    if (message.includes("diaper") || message.includes("poop") || message.includes("wet")) {
...
${avgDiapersPerDay >= 6 ? "Your diaper change frequency looks healthy! 👍" : "Consider tracking more closely if you're concerned about frequency."}`
    }

    // General baby care questions
    if (message.includes("cry") || message.includes("fussy") || message.includes("calm")) {
...
Trust your instincts - you know ${babyName} best! 💕`
    ...
    if (message.includes("growth") || message.includes("milestone") || message.includes("development")) {
...
What's on your mind today?`,
  ...
  const getPatternInsights = (avgFeedings: number, avgSleep: number, avgDiapers: number): string => {
    const insights = []

    if (avgFeedings < 6) {
      insights.push("• Consider tracking feedings more closely - newborns typically need 8-12 feeds per day")
    } else if (avgFeedings > 12) {
      insights.push("• High feeding frequency - this could indicate growth spurts or cluster feeding")
    } else {
      insights.push("• Feeding frequency looks healthy! 👍")
    }

    if (avgSleep < 1) {
      insights.push("• Short sleep sessions are normal for newborns, but ensure total daily sleep is adequate")
    } else if (avgSleep > 4) {
      insights.push("• Great sleep duration! Longer stretches are developing well")
    }

    if (avgDiapers < 6) {
      insights.push("• Monitor hydration - babies typically need 6+ wet diapers per day")
    } else {
        ...
        <Card className="fixed bottom-6 right-6 w-[calc(100vw-2rem)] max-w-96 h-[70vh] max-h-[500px] md:w-96 md:h-[500px] shadow-2xl z-50 flex flex-col bg-white border-2 border-purple-200">
            ...
            {messages.map((message) => (
              ...
              >
                ...
                >
                  ...
                  <div
                    className={`text-xs mt-1 opacity-70 ${message.type === "user" ? "text-blue-100" : "text-gray-500"}`}
                  >
          ...
          <div className="p-3 md:p-4 border-t bg-white rounded-b-lg">
            ...
            <div className="text-xs text-gray-500 mt-2 text-center">
              AI responses are for informational purposes. Always consult your pediatrician for medical advice.
            </div>
          </div>
        </Card>
      )}
